...
void ControllerRun(Controller* me) {
    ...
    switch(me->state) {
        case CONTROLLER_PRE_LEVEL0: // Logic for state Pre_Level0
            if(me->t >= 5) {
                ...
                // Next state is Pre_Levelm
                state_u = CONTROLLER_PRE_LEVELM;
            }
            break;
        case CONTROLLER_PRE_LEVELM: // Logic for state Pre_Levelm
            if(me->t >= 0) {
                t_u = 0;
    ...
    switch(me->state) {
        case CONTROLLER_PRE_LEVEL0: // Intra-location logic for state Pre_Level0
            ...
            break;
        case CONTROLLER_PRE_LEVELM: // Intra-location logic for state Pre_Levelm
            t_u = me->t + 1 * STEP_SIZE;
